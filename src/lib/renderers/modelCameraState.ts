...
const pickNumber = (value: object, keys: string[]): number | null => {
...
export const formatFovDeg = (fovDeg: number): string =>
  `${formatNumber(fovDeg)}deg`;

export const readCameraState = (
  viewer: ModelViewerCameraLike,
): ModelCameraState | null => {
  ...
  const fovDeg = normaliseFovDeg(
    viewer.getFieldOfView?.() ?? viewer.fieldOfView,
  );
...
export const formatCameraState = (
  state: ModelCameraState | null,
): {
  cameraOrbit?: string;
  cameraTarget?: string;
  fieldOfView?: string;
