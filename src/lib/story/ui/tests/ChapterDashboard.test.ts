...
describe("Chapter dashboard navigation", () => {
  it("opens one focused task and returns to the dashboard", async () => {
    ...
    const instance = mount(ChapterOverlay, {
      ...
      props: {
        ...
        languages: ["en", "cy"],
        annotationTool,
        onSetAnnotationTool: (tool: ChapterAnnotationTool) =>
          annotationTool.set(tool),
      },
    });

    expect(
      target.querySelector("#chapter-dashboard-title")?.textContent,
    ).toContain("Chapter tools");
    const narration = target.querySelector(
      '[data-task-id="audio-timing"]',
    ) as HTMLElement;
    ...
    expect(
      target.querySelector('[data-task-id="transition-timing"]')?.textContent,
    ).toContain("Chapter transition time");
    const details = target.querySelector(
      '[data-task-id="details"] button',
    ...
    const back = detailsPanel.querySelector(
    ...
    const annotations = target.querySelector(
      '[data-task-id="focus"] button',
    ) as HTMLButtonElement;
    annotations.click();
    await tick();
    ...
    expect(annotationPanel.textContent).toContain("select one from the footer");
    expect(annotationPanel.textContent).not.toContain("Text box annotation");
    const rectangle = Array.from(
      annotationPanel.querySelectorAll(
        ".chapter-overlay__annotation-tools button",
      ),
    ).find((button) =>
      button.textContent?.includes("Rectangle"),
    ) as HTMLButtonElement;
    rectangle.click();
    await tick();
  ...
  it("keeps unavailable tasks visible with a reason and next action", () => {
    ...
    const comparison = target.querySelector(
      '[data-task-id="comparison"]',
    ) as HTMLElement;
    ...
    expect(
      (comparison.querySelector("button") as HTMLButtonElement).disabled,
    ).toBe(true);
    unmount(instance);
    target.remove();
  });
